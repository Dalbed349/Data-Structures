...
// loop commented out for test2.js
...
request('https://parsons.nyc/aa/m01.html', function(error, response, body){
    if (!error && response.statusCode == 200) {
        fs.writeFileSync('/home/ec2-user/environment/data/m01.txt', body);  // 1873 lines   // '/home/ec2-user/environment/data/thesis' + AAlist[i] + '.txt'  /// Thesis[undefined'.txt]
    }
...
request('https://parsons.nyc/aa/m02.html', function(error, response, body){
    if (!error && response.statusCode == 200) {
        fs.writeFileSync('/home/ec2-user/environment/data/m02.txt', body);  // 2430 lines   // only changing m01 and m01.txt // common denominator among file names. set this up as [i] ?
    }
    else {console.log("Request failed!")}
});
