...
$('tr tr tr').each(function(i, outer_elem) {
    ...
    $(outer_elem).find('td').eq(0).each(function(i, elem) {
        ...
        if ($(elem).attr('style') === 'border-bottom:1px solid #e3e3e3; width:260px') {
            ...
            Address.street_address = $(elem).html().split('<br>')[2].trim().split(",")[0]   /// .split(",")[0] works for file 6,7 
            
            
                                                            ///.match(/^\d{1,5}.+?(?=,|-)/gm)[0]   /// works for 1,2,3,4,5,8,9,10/// MAP6 AND 7 THIS NEEDS TO CHANGE
...
console.log(aa.table[8].meetings_list)
fs.writeFileSync(filename+".json", JSON.stringify(aa.table));